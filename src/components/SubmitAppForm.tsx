...
import { useHasPaidForSubmission } from '@/hooks/useHasPaidForSubmission';
import { getLightningPaymentConfig } from '@/lib/lightningPaymentConfig';
import { Plus, X, Globe, Smartphone, Monitor, Zap, Coins, Check } from 'lucide-react';
...
export function SubmitAppForm() {
  ...
  const paymentConfig = getLightningPaymentConfig();
  ...
  const onSubmit = (data: AppFormData) => {
    ...
    if (paymentConfig.paymentRequired && !paymentStatus?.hasPaid) {
    ...
    // Proceed with direct submission (no payment required)
    publishAppSubmission({ content, tags });
  };

  const publishAppSubmission = ({ content, tags }: { content: string; tags: string[][] }) => {
    publishEvent(
      {
        kind: 31990,
        content,
  ...
  const handlePaymentSuccess = () => {
    if (pendingSubmission) {
      publishAppSubmission(pendingSubmission);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          {/* Payment Information */}
          {paymentConfig.paymentRequired && !paymentStatus?.hasPaid && (
            <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
              <div className="flex items-center space-x-2 text-blue-800 dark:text-blue-200">
                <Coins className="h-4 w-4" />
                <span className="font-medium text-sm">
                  Payment Required: {paymentConfig.feeSats} sats
                </span>
              </div>
              <p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
                A one-time payment is required to submit your app. After payment, you can edit your app as many times as you want.
              </p>
            </div>
          )}
          ...
          {paymentStatus?.hasPaid && (
            <div className="mt-3 p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg">
              ...
              <p className="text-xs text-green-600 dark:text-green-300 mt-1">
                You can submit and edit apps without additional payment.
              </p>
            </div>
          )}
        </CardHeader>
